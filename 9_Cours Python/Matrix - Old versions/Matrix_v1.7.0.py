...
############################################################################################################################## Def main variables
charexcept = -1
grid = np.full((m,n),charexcept,int)
...
############################################################################################################################## Building evolutive grid
    for j in range (m):
        for i in range (n):
            if gridtemp[i,j]!=charexcept:                #If drop there is
                ...
                if j==0:                                                # If first line -> Generate one and a length
                    ...
                    if gridtemp[i,j-1]!=charexcept:              #If drop in previous cell -> new drop and get length
                        grid[i,j]=random.randint(0,9)
...
############################################################################################################################## Display
    for i in range (n):
        for j in range (m):
            canvas.create_text(i*width/n, j*height/m+5, text=(grid[i,j] if grid[i,j]!=charexcept else None), font="Arial 8", fill=colors[droplength[i,j,1]])
    canvas.pack()
